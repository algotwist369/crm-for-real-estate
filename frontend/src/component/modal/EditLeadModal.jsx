...
import { useUpdateLead } from "../../hooks/useLeadHooks";
...
const EditLeadModal = ({ isOpen, onClose, onUpdate, lead }) => {
    ...
    useEffect(() => {
        if (lead && isOpen) {
            // Find matched standard requirements
            const currentReqs = lead.requirement ? lead.requirement.split(",").map(r => r.trim()) : [];
            const standardReqs = currentReqs.filter(r => REQUIREMENTS.includes(r));
            const otherReqs = currentReqs.filter(r => !REQUIREMENTS.includes(r));

            const budgetStr = lead.budget || "";
            let foundCurrency = lead.currency || "₹"; // Handle backend currency natively if it exists
            
            // Fallback parsing for previously concatenated strings
            if (!lead.currency) {
                if (budgetStr.startsWith("$")) foundCurrency = "$";
                else if (budgetStr.startsWith("AED")) foundCurrency = "AED";
            }

            const isCustom = budgetStr ? !getBudgetOptions(foundCurrency).includes(budgetStr) : false;
            let displayBudget = budgetStr;
            if (isCustom && budgetStr.startsWith(foundCurrency)) {
                displayBudget = budgetStr.replace(foundCurrency, "").trim();
            }
            ...
            setFormData({
                ...
                otherRequirement: otherReqs.join(", ") || "",
                budget: displayBudget,
                currency: foundCurrency,
                isCustomBudget: isCustom,
                source: lead.source || SOURCES[0],
                properties: lead.properties && lead.properties.length > 0 ? lead.properties.map(p => p._id) : [],
                priority: (lead.priority || "Medium").charAt(0).toUpperCase() + (lead.priority || "Medium").slice(1),
                clientType: (lead.client_type || "Buying").charAt(0).toUpperCase() + (lead.client_type || "Buying").slice(1),
            });
        }
    }, [lead, isOpen]);
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            properties: formData.properties // Send direct array of selected property _ids
        };
        ...
        <div className="fixed inset-0 bg-zinc-950/80 z-[60] flex items-center justify-center p-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded w-full max-w-xl overflow-hidden shadow-lg flex flex-col max-h-[95vh]">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
                    <div>
                        <h2 className="text-lg font-medium text-white">Edit Lead Profile</h2>
                        <p className="text-xs text-zinc-500 mt-0.5">Update client requirements and priorities</p>
                    </div>
